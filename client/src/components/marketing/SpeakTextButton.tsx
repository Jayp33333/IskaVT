...
export function SpeakTextButton({ text, className = "" }: SpeakTextButtonProps) {
  ...
  return (
    <button
      ...
      className={`inline-flex items-center justify-center rounded-xl border-2 border-ink bg-gold p-2.5 text-ink transition-colors hover:bg-gold/90 sm:rounded-2xl sm:p-3 ${className}`}
    >
      {isSpeaking ? (
