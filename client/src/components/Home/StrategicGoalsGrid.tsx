    ...
    <div className="space-y-5 sm:space-y-6">
      {strategicGoalsContent.map((pillar, pillarIndex) => (
        <motion.article
          ...
          className="overflow-hidden rounded-2xl border border-black bg-[#FFFDF5] shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] sm:rounded-3xl"
        >
          <div
            className={`flex items-center gap-3 border-b border-black px-4 py-3 sm:gap-4 sm:border-b sm:px-5 sm:py-4 ${pillar.accentClass}`}
          >
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-black bg-white text-sm font-black shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] sm:h-11 sm:w-11 sm:rounded-2xl sm:text-base">
              {pillar.number}
            </div>
            <div>
          ...
          <ul className="divide-y-2 divide-black/10">
            {pillar.goals.map((goal, goalIndex) => (
              ...
              >
                <span className="mt-0.5 shrink-0 rounded-lg border border-black bg-[#800000] px-2 py-1 text-[10px] font-black uppercase text-white sm:text-xs">
                  {goal.id}
                </span>
