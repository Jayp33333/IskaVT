    ...
    <AnimatePresence>
      {currentZones.length > 0 && (
        <motion.div
          ...
          className="fixed left-1/2 top-[clamp(54px,8vh,86px)] z-200 max-w-[min(46vw,440px)] -translate-x-1/2 whitespace-normal break-words rounded-3xl border-[3px] border-ink bg-cream/95 px-[clamp(12px,3vw,28px)] py-[clamp(5px,1.2vw,10px)] text-center text-[clamp(12px,2.2vw,22px)] font-black italic leading-tight tracking-wide text-maroon shadow-brutal-sm pointer-events-none select-none backdrop-blur-sm [@media(max-width:768px)]:max-w-[62vw] [@media(max-width:480px)]:top-[calc(18vh+2rem)] [@media(max-width:480px)]:max-w-[72vw] [@media(max-height:500px)]:top-[3.4rem] [@media(max-height:500px)]:max-w-[52vw] [@media(max-height:500px)]:border-2 [@media(max-height:500px)]:shadow-brutal-sm [@media(orientation:landscape)_and_(max-height:600px)]:top-2 [@media(orientation:landscape)_and_(max-height:600px)]:max-w-[44vw] [@media(orientation:landscape)_and_(max-height:600px)]:rounded-xl [@media(orientation:landscape)_and_(max-height:600px)]:border-2 [@media(orientation:landscape)_and_(max-height:600px)]:px-3 [@media(orientation:landscape)_and_(max-height:600px)]:py-1.5 [@media(orientation:landscape)_and_(max-height:600px)]:text-[11px] [@media(orientation:landscape)_and_(max-height:600px)]:leading-none [@media(orientation:landscape)_and_(max-height:600px)]:shadow-brutal-sm"
          style={{
