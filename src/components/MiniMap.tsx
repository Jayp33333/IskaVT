...
export function MiniMap({ showMiniMap }: MiniMapProps) {
  ...
  const characterPosition = useWorld((state: any) => state.characterPosition) as THREE.Vector3;
  const pinPosition = useWorld((state: any) => state.pinPosition) as THREE.Vector3 | null;
  const setPinPosition = useWorld((state: any) => state.setPinPosition) as (pos: THREE.Vector3) => void;
  const selectPin = useWorld((state: any) => state.selectPin) as boolean;
  const isPinConfirmed = useWorld((state: any) => state.isPinConfirmed) as boolean;
  const currentZoom = useWorld((state: any) => state.currentZoom) as number;
  const setCurrentZoom = useWorld((state: any) => state.setCurrentZoom) as (zoom: number) => void;

  const [isDragging, setIsDragging] = useState(false);
  const [dragStart, setDragStart] = useState({ x: 0, y: 0 });
  const [cameraPosition, setCameraPosition] = useState(new THREE.Vector3(0, zoomLevel, 0));
  ...
  useEffect(() => {
    ...
    if (showMiniMap) {
      setCameraPosition(new THREE.Vector3(characterPosition.x, currentZoom, characterPosition.z));
    } else {
      setCameraPosition(
        new THREE.Vector3(
          characterPosition.x,
  ...
  // Update camera height when zoom changes
  useEffect(() => {
    if (showMiniMap) {
  ...
  const handlePointerMove = (event: PointerEvent) => {
    ...
    const dragSensitivity = 0.01 * (currentZoom / zoomLevel);
  ...
  const handleMapClick = (event: PointerEvent) => {
    ...
    if (raycaster.ray.intersectPlane(groundPlane, intersectionPoint)) {
      setPinPosition(new THREE.Vector3(intersectionPoint.x, 2, intersectionPoint.z));
      console.log(
        `Pin placed at: ${intersectionPoint.x.toFixed(2)}, ${intersectionPoint.z.toFixed(2)}`
      );
    }
  };
      ...
      {pinPosition && (
        ...
        >
          <mesh>
            ...
            <meshBasicMaterial map={pinTexture} transparent depthTest={false} />
          </mesh>
        </Billboard>
      )}
